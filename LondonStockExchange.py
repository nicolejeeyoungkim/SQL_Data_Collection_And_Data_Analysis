...
conn = psycopg2.connect("host=XXX dbname=XXX user=data password=XXX")
cur = conn.cursor()
...
for i in range(121):
    print i    
    param = {'fl':'_id,snippet,headline,pub_date', 'api-key':'ca5c6afbd44d4c09ac938a51c09a6fda', 'q': 'London Stock Exchange','begin_date':'20150601', 'sort': 'oldest', 'page': i}
    ...
    for num in range(10):
        ...
        if only==True:
            cur.execute("INSERT INTO newssearchLondonstockexchange (PubDate, Headline, leadParagraph, NewsID) VALUES (%s,%s,%s,%s);",[pubDate, headline, snippet, _id])
            conn.commit()
